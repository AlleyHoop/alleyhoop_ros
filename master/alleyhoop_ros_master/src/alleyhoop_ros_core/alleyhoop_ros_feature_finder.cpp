...
namespace AlleyHoopROSCore
{
	...
	: AlleyHoopMVC::Model(), nh(*_nh)
    {
        ...
        std::string blob_feature_finder_service;
        if(!nh.getParam("AlleyHoop_Models/BlobFeaturesOnImage_ServiceName",blob_feature_finder_service))
            blob_feature_finder_service = "example_feature_finder";
        ...
        blob_feature_finder_client = nh.serviceClient<alleyhoop_ros_msgs::FindFeaturesOnImage>(blob_feature_finder_service);
    ...
    bool FeatureFinder::findBlobOnImage(std::list<AlleyHoopROSUtils::Feature*>& features, cv_bridge::CvImagePtr& imagePtr)
    { 
        ...
        if (!blob_feature_finder_client.call(srv))
        {
            if(verboseMode)
                ROS_ERROR("FeatureFinder: Failed to call service for blob feature finder");
            return false;
        }
        ...
        if(srv.response.step < 1)
        {
            if(verboseMode)
            {
                std::cout << "FeatureFinder: no blob features found" << std::endl;
            }
            return false;
        }
        
        //read response
        if(verboseMode) std::cout << "FeatureFinder: found " << srv.response.features.size()/srv.response.step << " blob features. Size: " << srv.response.features.size() << " Step: " << srv.response.step << std::endl;
        if(srv.response.step == 4)
        {   
            for(size_t i = 0; i < srv.response.features.size(); i+=srv.response.step)
            {
                
                if(verboseMode) std::cout << "FeatureFinder: found blob feature at (" << srv.response.features[i] << "," << srv.response.features[i+1] << "," << srv.response.features[i+2] << "," << srv.response.features[i+3] << ")" << std::endl;
